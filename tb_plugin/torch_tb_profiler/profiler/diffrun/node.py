...
class OperatorRunNode(LogicalNode):
    def __init__(self, nodes) -> None:
        ...
        if isinstance(nodes, OperatorNode):
            super().__init__(nodes.name, nodes.get_device_start_time(), nodes.get_device_end_time())
        elif isinstance(nodes, list):
            super().__init__("multiple nodes", nodes[0].get_device_start_time(), nodes[-1].get_device_end_time())
        self.op_nodes = nodes
...
class DiffNode:
    ...
    def _build_tree(self):
        ...
        if isinstance(self.left_node.op_nodes, OperatorNode) and isinstance(self.right_node.op_nodes, OperatorNode):
            # simple node match.
            diff_nodes = compare_operator_nodes(self.left_node.op_nodes.children, self.right_node.op_nodes.children, self.level + 1)
            if diff_nodes:
